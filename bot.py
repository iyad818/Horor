...
def generate_cards(message):
    try:
        ...
        if len(parts) < 2:
            bot.send_message(message.chat.id, "Oops! That Doesn't Look Like a Valid BIN. Example: `/gen BIN Amount` or `/gen BIN|MM|YY|CVV`", parse_mode="Markdown")
            return

        bin_input = parts[1]
        amount = int(parts[2]) if len(parts) > 2 else 10

        # تحقق من عدد البطاقات المطلوب إذا كان أكثر من 50
        if amount > 50:
            bot.send_message(message.chat.id, "❌ You can only generate up to 50 cards at a time. Please reduce the number and try again.")
            return
        ...
        # Load BIN data and flags using ujson
        with open('top_bin.json', 'r') as f:
            bin_data = ujson.load(f)

        with open('flags.json', 'r') as f:
            flags_data = ujson.load(f)
        ...
        # تابع توليد بطاقات باستخدام Luhn
        ...
        # تحقق من صلاحية البطاقة باستخدام خوارزمية Luhn
        ...
        # تحقق عدة مرات من صلاحية البطاقة
        def verify_card_luhn(card_number):
            retries = 3  # عدد المحاولات للتحقق
            for _ in range(retries):
                if validate_luhn(card_number):
                    return True
            return False

        # توليد عدد من البطاقات الصالحة
        def generate_valid_cards():
            valid_cards = []
            attempts = 0
            while len(valid_cards) < amount:
                card = luhn_generate(bin_number)
                if verify_card_luhn(card):  # تحقق من صلاحية البطاقة
                ...
                attempts += 1
                if attempts > 1000:  # حالة الحماية من التكرار الغير محدود
                    break
            return valid_cards

        # إرسال رسالة انتظار سريعًا
        msg = bot.send_message(message.chat.id, "🔄 Generating cards… Please wait.", parse_mode="Markdown")

        # توليد البطاقات الصالحة
        cards = generate_valid_cards()

        # إذا كانت البطاقات أكثر من العدد المطلوب، حصرها في العدد المطلوب
        if len(cards) > 50:
            cards = cards[:50]

        # تحديد العدد الأقصى للبطاقات المرسلة كرسالة
        result_message = f"**Bin** → `{bin_number}`\n**Amount** → `{amount}`\n\n"

        # إضافة البطاقات في التنسيق المطلوب
        result_message += "\n".join([f"`{card}`" for card in cards]) + "\n\n"

        # إضافة معلومات إضافية
        result_message += f"𝐁𝐈𝐍 𝐈𝐧𝐟𝐨 ➜ `{bin_info.get('type', 'Unknown')} - {bin_info.get('category', 'Unknown')}`\n"
        result_message += f"𝐁𝐚𝐧𝐤 ➜ `{bin_info.get('bank', 'Unknown')}`\n"
        result_message += f"𝐂𝐨𝐮𝐧𝐭𝐫𝐲 ➜ `{bin_info.get('country', 'Unknown')} {flag}`\n"

        # تحديث رسالة الانتظار إلى رسالة البطاقات
        bot.edit_message_text(result_message, message.chat.id, msg.message_id, parse_mode="Markdown")
...
# إنشاء جدول المستخدمين الذين استخدموا الأكواد
cursor.execute('''
...
# دالة لإنشاء كود فريد
def generate_code():
    return str(random.randint(100000, 999999))

# دالة لحفظ الأكواد في قاعدة البيانات
def save_code_in_db(code, max_users, duration):
    cursor.execute('''
    INSERT INTO codes (code, max_users, duration) VALUES (?, ?, ?)
    ''', (code, max_users, duration))
    conn.commit()
...
def process_subscription_time(message, max_users):
    try:
        ...
        save_code_in_db(code, max_users, duration)
